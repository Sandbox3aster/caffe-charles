...
#include "caffe/layers/d_conv_layer.hpp"

namespace caffe {

template <typename Dtype>
void DConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void DConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  const vector<Blob<Dtype>*>& top) {
  const int first_spatial_axis = this->channel_axis_ + 1;
...
template <typename Dtype>
void DConvolutionLayer<Dtype>::compute_output_shape() {
...
template <typename Dtype>
void DConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void DConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(DConvolutionLayer);
#endif

INSTANTIATE_CLASS(DConvolutionLayer);
REGISTER_LAYER_CLASS(DConvolution);
}  // namespace caffe
