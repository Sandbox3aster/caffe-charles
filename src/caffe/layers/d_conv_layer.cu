...
*/
template <typename Dtype>
void DConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const Dtype* weight = bottom[1]->gpu_data();
  int weight_size = bottom[1]->count(1);
...
template <typename Dtype>
void DConvolutionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_LAYER_GPU_FUNCS(DConvolutionLayer);
